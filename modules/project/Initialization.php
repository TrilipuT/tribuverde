...
class Initialization extends AbstractModuleInitialization {
	...
	const CATEGORY = 'project_category';
	private $post_type;

	public function register_post_type() {
		$this->post_type = new PostType( self::POST_TYPE, __( 'Project', TEXT_DOMAIN ), [ 'name' => 'Проекти' ] );
		$this->post_type->set_supports( [ 'title', 'thumbnail', 'editor', 'excerpt' ] );
		$this->post_type->set_use_archive( 'projects' );
		$this->post_type->set_menu_icon( 'dashicons-video-alt' );
		$this->post_type->set_show_in_rest( true );
		$this->post_type->set_rewrite( [ 'slug' => 'project' ] );
		$this->post_type->set_use_archive( 'projects' );

		$tax = new Taxonomy( self::CATEGORY, 'Category' );
		$tax->set_rewrite( [ 'slug' => 'project-category' ] );
